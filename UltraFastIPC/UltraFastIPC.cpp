    ...
    void ProcessRequestUltraFast() {
        ...
        try {
            if (tokens[0] == "pe32_init") {
                ...
                int buffer;
                response = std::to_string(pe32_readl(bdn, offset, &buffer)) + ":" + std::to_string(buffer);
            }
            ...
            //Common functions
                ...
                char patternfileChar[256];
                strncpy(patternfileChar, tokens[4].c_str(), sizeof(patternfileChar) - 1);
            ...
            else if (tokens[0] == "pe32_set_fallingskew") {
                int bdn = std::stoi(tokens[1]);
                int pno = std::stoi(tokens[2]);
                int rt = std::stoi(tokens[3]);
